...
ggplot(data=train, aes(x =log(CAPITAL))) + 
	geom_density()

data(iris)
test <- sample(1:150,20)
kpc <- kpca(~.,data=train[,-2],kernel="rbfdot",
kpar=list(sigma=0.2),features=2)
#print the principal component vectors
pcv(kpc)
#plot the data projection on the components
ggplot(data=data.frame(rotated(kpc),train["agree"]), aes(X1, X2, colour=factor(agree))) + 
...
calc_k( train[,-2], m=2, 1, 15 )
k5 <- lle(train[,-2], m=2, k=100, reg=2, ss=FALSE, id=TRUE, v=0.9 )
ggplot(data=data.frame(k5$Y,train["agree"]), aes(X1, X2, colour=factor(agree))) + 
	geom_point(size=3, alpha=.6)
...
ggplot(data=iris, aes(x=Sepal.Length, y=Sepal.Width,colour=factor(Species))) + 
	geom_point(size=3, alpha=.6)
